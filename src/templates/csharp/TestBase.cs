...
namespace AppiumTest
{
    public class TestBase
    {
        ...
        public virtual void Setup(AppiumOptions desiredCaps, double retinaScale)
        {
            ...
            this.isIos = MobilePlatform.IOS.Equals(desiredCaps.PlatformName);
        ...
        {
            string script = File.ReadAllText("execute-script-on-web-element.js", Encoding.UTF8);
        ...
        public void SwipeByPoint(Point fromPoint, Point toPoint, int durationInMs)
        {
            ...
            swipe.Press(fromPoint.X, fromPoint.Y).Wait(durationInMs).MoveTo(toPoint.X, toPoint.Y).Release();
        ...
        public void SendKeys(string keys)
        {
            ...
            { 
                ...
                {
                    int codePoint = Char.ConvertToUtf32(keys, index);
                    GetAndroidDriver().PressKeyCode(codePoint);
        ...
        public void UpdateSettings()
        {
            if (!this.isIos)
            {
                GetAndroidDriver().SetSetting("ignoreUnimportantViews", true);
            }
        }
